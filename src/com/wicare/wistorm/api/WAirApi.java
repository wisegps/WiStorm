...
public class WAirApi extends WiStormAPI{
	...
	public WAirApi(Handler uiHandler){
	...
	private Callback handleCallBack = new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			case Msg.SET_AIR_SPEED_COMMAND:
				...
				break;
			case Msg.SET_AIR_SWITCH_COMMAND:
				Log.i("FragmentHomeAir", "控制开关返回信息: " + msg.toString());
				break;
			case Msg.SET_AIR_MODEL_COMMAND:
	...
	public void getToken(String account, String password) {
		...
		volley.request(url, Msg.GET_NEW_API_TOKEN);
	}
	...
	public void setAirCommand(String token,String device_id,String command,String model){
		...
		if(model.equals(Msg.SPEED_COMMAND_MODEL)){
			volley.request(url, Msg.SET_AIR_SPEED_COMMAND);
		}else if(model.equals(Msg.SWITCH_COMMAND_MODEL)){
			volley.request(url, Msg.SET_AIR_SWITCH_COMMAND);
		}else if(model.equals(Msg.MODEL_SET_COMMAND_MODEL)){
			volley.request(url, Msg.SET_AIR_MODEL_COMMAND);
		}
	}
